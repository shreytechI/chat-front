...
export const ChatBubble: React.FC<ChatBubbleProps> = ({ message, isOwn }) => {
  return (
    <div className={isOwn ? "flex justify-end group relative" : "flex justify-start group relative"}>
      <div
        className={twMerge(
          "relative max-w-xs sm:max-w-md lg:max-w-lg xl:max-w-xl px-4 py-3 shadow-lg",
          isOwn
            ? "bg-violet-600 text-white rounded-tl-2xl rounded-tr-2xl rounded-bl-2xl rounded-br-md"
            : "bg-white text-black rounded-tl-2xl rounded-tr-2xl rounded-br-2xl rounded-bl-md"
        )}
      >
        {/* Files */}
      ...
      <button
        className={twMerge(
          "absolute top-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 p-1 rounded-full hover:bg-black/10",
